...
import { DummyInterface, fetchDummy } from "@/lib/api";
...
const columns: Column<DummyInterface>[] = [
...
const filters: FilterDef<DummyInterface>[] = [
  {
    key: "status",
    label: "Status",
    options: [
...
export default function DummyPage() {
  ...
  const [rows, setRows] = useState<DummyInterface[]>([]);
  ...
  const load = useCallback(async (mode: "load" | "refresh" = "load") => {
    ...
    try {
      ...
      setErrorMsg(e instanceof Error ? e.message : "Failed to load dummy interfaces.");
      setStatus("error");
    }
  }, [deviceId]);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-[36px] pt-[28px] pb-5 flex-shrink-0">
        ...
        {device && <p className="text-[13px] text-[var(--qz-fg-4)] mt-1">{"Always-up virtual interfaces for stable addressing"}</p>}
      </div>

      <div className="flex-1 overflow-auto px-[36px] pb-[28px]">
        {status === "loading" && (
          <div className="text-[13px] text-[var(--qz-fg-4)]">Loading dummy interfaces…</div>
        )}
        {status === "error" && (
        ...
        {status === "ready" && (
          <DataTable
            ...
            filters={filters}
            rowId={(r) => r.name}
            searchPlaceholder="Search dummy interfaces…"

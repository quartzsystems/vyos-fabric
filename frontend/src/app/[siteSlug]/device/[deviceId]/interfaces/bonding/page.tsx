...
import { BondingInterface, fetchBonding } from "@/lib/api";
...
const columns: Column<BondingInterface>[] = [
  ...
  { key: "mode", header: "Mode", value: (r) => r.mode ?? "", mono: true, sortable: true, width: 140 },
  {
    key: "members",
    header: "Members",
...
const filters: FilterDef<BondingInterface>[] = [
  {
    key: "status",
    label: "Status",
    options: [
...
export default function BondingPage() {
  ...
  const [rows, setRows] = useState<BondingInterface[]>([]);
  ...
  const load = useCallback(async (mode: "load" | "refresh" = "load") => {
    ...
    try {
      setRows(await fetchBonding(deviceId));
      setStatus("ready");
    } catch (e) {
      setErrorMsg(e instanceof Error ? e.message : "Failed to load bonding interfaces.");
      setStatus("error");
    }
  }, [deviceId]);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-[36px] pt-[28px] pb-5 flex-shrink-0">
        ...
        {device && <p className="text-[13px] text-[var(--qz-fg-4)] mt-1">{"Link aggregation (bonding) interfaces"}</p>}
      </div>

      <div className="flex-1 overflow-auto px-[36px] pb-[28px]">
        {status === "loading" && (
          <div className="text-[13px] text-[var(--qz-fg-4)]">Loading bonding interfaces…</div>
        )}
        {status === "error" && (
        ...
        {status === "ready" && (
          <DataTable
            ...
            emptyMessage="No bonding interfaces configured."
            toolbar={<Button kind="primary" size="sm" icon={Plus}>Create bond</Button>}
            actions={() => <RowActions />}

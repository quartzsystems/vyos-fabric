...
import { MacsecInterface, fetchMacsec } from "@/lib/api";
...
const columns: Column<MacsecInterface>[] = [
...
const filters: FilterDef<MacsecInterface>[] = [
  {
    key: "status",
    label: "Status",
    options: [
...
export default function MacsecPage() {
  const { deviceId, device } = useDevice();
  const [rows, setRows] = useState<MacsecInterface[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setErrorMsg(e instanceof Error ? e.message : "Failed to load MACsec interfaces.");
      setStatus("error");
    }
  }, [deviceId]);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-[36px] pt-[28px] pb-5 flex-shrink-0">
        ...
        {device && <p className="text-[13px] text-[var(--qz-fg-4)] mt-1">{"Encrypted Layer 2 (MACsec) over a parent interface"}</p>}
      </div>

      <div className="flex-1 overflow-auto px-[36px] pb-[28px]">
        {status === "loading" && (
          <div className="text-[13px] text-[var(--qz-fg-4)]">Loading MACsec interfaces…</div>
        )}
        {status === "error" && (
        ...
        {status === "ready" && (
          <DataTable
            ...
            filters={filters}
            rowId={(r) => r.name}
            searchPlaceholder="Search MACsec interfaces…"

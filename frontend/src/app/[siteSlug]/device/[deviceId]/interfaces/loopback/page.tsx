...
import { LoopbackInterface, fetchLoopback } from "@/lib/api";
...
export default function LoopbackPage() {
  ...
  const load = useCallback(async () => {
    setStatus("loading");
    try {
      setRows(await fetchLoopback(deviceId));
      setStatus("ready");
    } catch (e) {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-[36px] pt-[28px] pb-5 flex-shrink-0">
        ...
        {device && <p className="text-[13px] text-[var(--qz-fg-4)] mt-1">{"Loopback interfaces for stable local addressing"}</p>}
      </div>
      ...
      <div className="flex-1 overflow-auto px-[36px] pb-[28px]">
        ...
        {status === "ready" && (
          <DataTable
            ...
            emptyMessage="No loopback interfaces configured."
            toolbar={<Button kind="primary" size="sm" icon={Plus}>Create loopback</Button>}
            actions={() => <RowActions />}
          />
        )}
      </div>

...
import { PppoeInterface, fetchPppoe } from "@/lib/api";
...
const columns: Column<PppoeInterface>[] = [
...
const filters: FilterDef<PppoeInterface>[] = [
  {
    key: "status",
    label: "Status",
    options: [
...
export default function PppoePage() {
  ...
  const [rows, setRows] = useState<PppoeInterface[]>([]);
  ...
  const load = useCallback(async () => {
    setStatus("loading");
    try {
      setRows(await fetchPppoe(deviceId));
      setStatus("ready");
    } catch (e) {
      setErrorMsg(e instanceof Error ? e.message : "Failed to load PPPoE interfaces.");
      setStatus("error");
    }
  }, [deviceId]);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-[36px] pt-[28px] pb-5 flex-shrink-0">
        ...
        {device && <p className="text-[13px] text-[var(--qz-fg-4)] mt-1">{"PPP over Ethernet client interfaces"}</p>}
      </div>

      <div className="flex-1 overflow-auto px-[36px] pb-[28px]">
        {status === "loading" && (
          <div className="text-[13px] text-[var(--qz-fg-4)]">Loading PPPoE interfaces…</div>
        )}
        {status === "error" && (
        ...
        {status === "ready" && (
          <DataTable
            ...
            filters={filters}
            rowId={(r) => r.name}
            searchPlaceholder="Search PPPoE interfaces…"
            emptyMessage="No PPPoE interfaces configured."
            toolbar={<Button kind="primary" size="sm" icon={Plus}>Create PPPoE</Button>}
            actions={() => <RowActions />}
          />
        )}
      </div>

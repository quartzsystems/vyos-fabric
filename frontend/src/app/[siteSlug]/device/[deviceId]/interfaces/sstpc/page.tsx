...
import { SstpcInterface, fetchSstpc } from "@/lib/api";
...
const columns: Column<SstpcInterface>[] = [
...
const filters: FilterDef<SstpcInterface>[] = [
  {
    key: "status",
    label: "Status",
    options: [
...
export default function SstpcPage() {
  const { deviceId, device } = useDevice();
  const [rows, setRows] = useState<SstpcInterface[]>([]);
  ...
  const load = useCallback(async (mode: "load" | "refresh" = "load") => {
    ...
    try {
      ...
      setErrorMsg(e instanceof Error ? e.message : "Failed to load SSTP client interfaces.");
      setStatus("error");
    }
  }, [deviceId]);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-[36px] pt-[28px] pb-5 flex-shrink-0">
        ...
        {device && <p className="text-[13px] text-[var(--qz-fg-4)] mt-1">{"Secure Socket Tunneling Protocol client interfaces"}</p>}
      </div>

      <div className="flex-1 overflow-auto px-[36px] pb-[28px]">
        {status === "loading" && (
          <div className="text-[13px] text-[var(--qz-fg-4)]">Loading SSTP client interfaces…</div>
        )}
        {status === "error" && (
        ...
        {status === "ready" && (
          <DataTable
            ...
            filters={filters}
            rowId={(r) => r.name}
            searchPlaceholder="Search SSTP client interfaces…"

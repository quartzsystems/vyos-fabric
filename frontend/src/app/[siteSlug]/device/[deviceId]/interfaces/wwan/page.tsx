...
import { WwanInterface, fetchWwan } from "@/lib/api";
...
const columns: Column<WwanInterface>[] = [
...
const filters: FilterDef<WwanInterface>[] = [
  {
    key: "status",
    label: "Status",
    options: [
...
export default function WwanPage() {
  const { deviceId, device } = useDevice();
  const [rows, setRows] = useState<WwanInterface[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setErrorMsg(e instanceof Error ? e.message : "Failed to load WWAN interfaces.");
      setStatus("error");
    }
  }, [deviceId]);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-[36px] pt-[28px] pb-5 flex-shrink-0">
        ...
        {device && <p className="text-[13px] text-[var(--qz-fg-4)] mt-1">{"Wireless WAN (cellular modem) interfaces"}</p>}
      </div>

      <div className="flex-1 overflow-auto px-[36px] pb-[28px]">
        {status === "loading" && (
          <div className="text-[13px] text-[var(--qz-fg-4)]">Loading WWAN interfaces…</div>
        )}
        {status === "error" && (
        ...
        {status === "ready" && (
          <DataTable
            ...
            filters={filters}
            rowId={(r) => r.name}
            searchPlaceholder="Search WWAN interfaces…"

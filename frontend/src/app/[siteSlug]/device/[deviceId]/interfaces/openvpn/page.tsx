...
import { OpenvpnInterface, fetchOpenvpn } from "@/lib/api";
...
const columns: Column<OpenvpnInterface>[] = [
  ...
  },
  {
    key: "remote_host",
    header: "Remote",
    value: (r) => r.remote_host ?? "",
    render: (r) => r.remote_host ?? "—",
    mono: true,
  },
  {
    key: "addresses",
...
const filters: FilterDef<OpenvpnInterface>[] = [
  {
    key: "status",
    label: "Status",
    options: [
...
export default function OpenvpnPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setErrorMsg(e instanceof Error ? e.message : "Failed to load OpenVPN interfaces.");
      setStatus("error");
    }
  }, [deviceId]);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-[36px] pt-[28px] pb-5 flex-shrink-0">
        ...
        {device && <p className="text-[13px] text-[var(--qz-fg-4)] mt-1">{"OpenVPN tunnel interfaces (site-to-site, client, server)"}</p>}
      </div>

      <div className="flex-1 overflow-auto px-[36px] pb-[28px]">
        {status === "loading" && (
          <div className="text-[13px] text-[var(--qz-fg-4)]">Loading OpenVPN interfaces…</div>
        )}
        {status === "error" && (
        ...
        {status === "ready" && (
          <DataTable
            ...
            filters={filters}
            rowId={(r) => r.name}
            searchPlaceholder="Search OpenVPN interfaces…"
